...
multi_core_function <- function(samples_no){
  ...
  n_markers <- sapply(floor(0.91 * lengths(markers$L)), min, ncol(Y)/K)
  ...
  output_file <- paste0('~/IndependentStudy/Data/dtangle_091/', as.character(samples_no), '_signature_filter.tsv')
#   print(output_file)
