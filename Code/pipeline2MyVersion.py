...
def trim_single_exp(study):
    ...
    """
    a = pd.read_csv(decon_temp + 'results_Normal_'
            + study + '.tsv', sep = '\t')
    a = a[:1]
    a.to_csv(decon_temp + 'results_Normal_'
            + study + '.tsv', sep = '\t')
    
    b = pd.read_csv(decon_temp + 'results_Weight_'
            + study + '.tsv', sep = '\t')
    b = b[:1]
    b.to_csv(decon_temp + 'results_Weight_'
            + study + '.tsv', sep = '\t')

######## OVERALL PIPELINE

def decon_study(study, test_dataset = False, TM = False, TrainedNoiseModel_1 = None, TrainedNoiseModel_2 = None):
    """
...
#     # Deconvolution
    ...
    if TM == True:
        result = deconrnaseqweightCore(query_file, reference_file, use_scale = False, 
                                       Trainmodel_1 = TrainedNoiseModel_1, Trainmodel_2 = TrainedNoiseModel_2)
