...
#             continue
    ...
    if weightedApproach == 'CVd':
        ...
        if provideTM != None:
            tempBB = list(BB)
            for i in range(len(logBB)):
                if logBB[i] <= 31: # To solve overflow issue.
                    tempBB[i] = math.exp(provideTM(logBB[i]))
                else:
                    tempBB[i] = math.exp(provideTM(31))
#                 if tempBB[i] != 0:
#                     tempBB[i] = math.exp(provideTM(logBB[i]))
#                 else:
#                     tempBB[i] = math.exp(provideTM(logBB[i]))
            ...
            return "Please provide the required training model."
    elif weightedApproach == 'Signature_std': # Not make sense actually 
        if provideTM != None:
            tempBB = list(BB)
            for i in range(len(tempBB)):
                if tempBB[i] != 0:
                    tempBB[i] = math.exp(provideTM(logBB[i]))/scipy.special.expit(math.exp(provideTM(logBB[i])))
                else:
                    tempBB[i] = math.exp(provideTM(logBB[i]))/scipy.special.expit(math.exp(provideTM(logBB[i])))
            tempBB = np.array(tempBB)
            weightedUse = 1/tempBB
...
def deconrnaseqweightIRLS(datasets, signatures, checksig = False, deconMethod = 'CVd',
    ...
    weightMatrix = buildweightmatrix1(data_subdata, signaturenp, 
