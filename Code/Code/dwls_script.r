...
sig <- read.table('./mixture_dtangle.txt', sep='\t', row.names = 1, header = TRUE)
bulk <- read.table('./dwls_signature.txt', sep='\t', row.names = 1, header = TRUE)
# bulk_t <- transpose(bulk)
# colnames(bulk_t) <- rownames(bulk)
# rownames(bulk_t) <- colnames(bulk)
# bulk <- bulk_t

sig_m <- data.matrix(sig)
bulk_m <- data.matrix(bulk_exp)
out.all <- solveDampenedWLS(sig_m, bulk_m)
...
#   sig_m <- data.matrix(sig)
#   bulk_m <- data.matrix(bulk_exp)
#   out.all <- rbind(out.all, solveDampenedWLS(sig_m, bulk_m))
# }

out.all[out.all<0] <- 0
write.csv(out.all, file = "./dwls_result.txt")